...
The number of roots is 4 for each PES output file and 10 for each VEE file. 
"""
roots_vee = 10 
roots_pes = 4
atom1 = 4
...
for filename in os.listdir("PES_S0_DTE"):
    states = []
    states_op = [] 
    if filename.endswith(".out"):

        with open(os.path.join("PES_S0_DTE",filename), 'r+') as file: 
            for line in file:  
...
em_df.to_csv("DATA_PY/VEE_PES_S0", index = False, sep='\t')
print(em_df)
